...
def activate(lobby_id, sender, receiver=None, broker=None):
    """
    ...
    The  lobby_id  distinguishes YOUR simpleMQTT-enabled program from other
    ...
    Sender is the computer I am on.
    Receiver is my pal.
    Lobby_id is the server that we are connected too. We only see traffic with the same lobby id. 
    Broker is a third party server. When it is None, we run peer to pe
    """
    talker_to_broker = TalkerToBroker(sender, receiver, lobby_id, broker)
...
class Receiver:
    ...
    def __init__(self, controller, verbosity=2):
        """
...
class Broker:
    ...
    def __repr__(self):
        return f"Hostname {self.hostname} at ports {self.tcp_port} and {self.web_socket_port}"


class TalkerToBroker:
    """ A TalkerToBroker connects to a Broker and sets up communication. """

    def __init__(self, sender, receiver, lobby_id, broker=None):
        """
        ...
        The  lobby_id  distinguishes this simpleMQTT-enabled run from unrelated
